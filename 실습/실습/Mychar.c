...
void DrawMyBullet()  // 총알 발사기능
{
	int i;
	UPOINT ptpos, oldpos;
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE); // 핸들 선언
	for (i = 0; i < MAXENEMY_BULLET; i++)
	{
		if (myship_bullet[i].flag == TRUE)
		{
			...
			printf(" ");
			gotoxy(ptpos);
			SetConsoleTextAttribute(hConsole, 12); // 총알 색상변경
			printf("^");
...
void MyBulletshot(UPOINT ptthisMypos) // 총알발사기능
{
	int i;
